...
拾音 (ShiYin) 是一个 Linux 桌面端 GUI 音乐播放器，使用 Rust + Tauri v2 构建，支持网易云音乐和 QQ 音乐双音源，具备 Spotify 风格的现代界面、频谱可视化、歌词同步显示和动态主题色提取。

## 架构总览

- 后端：Rust workspace，包含 6 个 crate（core / player / sources / netease / qqmusic / cache）+ 1 个 Tauri 应用 crate
- 前端：React 18 + TypeScript + Vite + Tailwind CSS + Zustand 状态管理
- 音频引擎：GStreamer (gstreamer-rs)，独立线程运行，通过 tokio channel 与主线程通信
- IPC：Tauri v2 command/event 双向通信，所有调用携带 traceId 用于端到端链路追踪
- 持久化：tauri-plugin-store (配置) + SQLite/rusqlite (搜索缓存、歌词缓存) + 内存 LRU (热缓存)
...
| 模块路径 | 语言 | 职责 |
|----------|------|------|
| `crates/core` | Rust | 通用类型定义（Track, PlayerState, MusicSource trait, 错误类型） |
| `crates/player` | Rust | GStreamer 音频播放引擎，状态机管理，频谱分析 |
| `crates/sources` | Rust | 音乐源注册表 (SourceRegistry)，统一管理多音源 |
| `crates/netease` | Rust | 网易云音乐 API 客户端（weapi 加密、搜索、播放、歌词、歌单） |
| `crates/qqmusic` | Rust | QQ 音乐 API 客户端（签名计算、搜索、播放、歌词） |
| `crates/cache` | Rust | 内存 LRU 搜索缓存（5 分钟 TTL，128 容量） |
| `apps/rustplayer-tauri/src-tauri` | Rust | Tauri 应用入口，IPC commands，事件转发，SQLite 持久缓存，Cookie 存储，traceId 生成 |
| `apps/rustplayer-tauri/frontend` | TypeScript/React | 前端 UI（搜索、播放控制、歌词、频谱可视化、歌单、设置、错误处理） |
...
当前项目尚未包含测试文件。建议的测试方向：

- Rust 单元测试：weapi 加密 (`crates/netease/src/crypto.rs`)、QQ 音乐签名 (`crates/qqmusic/src/sign.rs`)、LRC 歌词解析、缓存 TTL 逻辑
...
- 路径别名：前端使用 `@/*` 映射到 `src/*`
- 状态管理：Zustand store 按功能拆分（player / ui / visualizer / toast / playlist）
- IPC 封装：所有 Tauri invoke 调用集中在 `frontend/src/lib/ipc.ts`，每次调用自动生成 traceId
...
- 新增音乐源时，实现 `MusicSource` trait 并在 `main.rs` 中 `registry.register()`
